...
In simple terms, this vulnerability allows a user to write arbitrary files as long as he has read right to these files. It is also interesting to note that Android phones are also affected. Android apps run with user rights, so a malicious or compromised app could take over the phone.
