...
Suppose the reference was encoded with a common encoder (e.g. `base64`). We can decode the original object reference, then encode a similar one.

On the other hand, the object reference may be hashed. If we look at the source code, we may see what is being hashed before the API call is made:
```javascript
$.ajax({
    url:"download.php",
    type: "post",
